...
char path[1024]="/cshome/vis/data/";
...
void init_mesh(){
  ...
  if(file){
    ...
    if(debug)
      ...
      fprintf(stderr, "reading image %d %d %d\n",w,h);
...
void init_bones(){
  ...
  while(fgets(line,1023,file)){
	...
	if(rs[ri]){
      ...
      if(debug)fprintf(stderr, "bn{%d}=\'%s\';bdof(%d)=%d;%% flipped:%d\n",i+1,name,i+1,bones[i]->ndof,flipped);
		 ...
		 int nrhs, const mxArray *prhs[])
{
  ...
  case mxCHAR_CLASS:
	...
	write(fd[1],&length,sizeof(uint32_t)); write(fd[1],&data,sizeof(uint32_t));
      }
      else if(strcmp(cmd,"points")==0){
	int ndims;
	const int * dims;
	if(nrhs<2)mexErrMsgTxt("Need points as 3 x n matrix");
  ...
  case mxDOUBLE_CLASS:
    {
      ...
      if(debug){
        fprintf(stderr, "ndims is %d: [",ndims);
      }
      for(;i<ndims;i++){
        if(debug){
